...
# 运算符优先权的比较，若输入运算符小于堆栈中的运算符，则返回值为1，否则返回0
# 在中序法表达式队列和暂存堆栈中，运算符的优先级表，其优先权值为INDEX/2
...
def infix_to_postfix():
    ...
    for flag in range(index+2):
        if infix_q[flag] == ")":   # 若输入为)，则输出堆栈内的运算符，直到堆栈内为(
            while stack_t[top] !="(":
                print("%c" %stack_t[top], end="")
                top -= 1
            top -= 1
            # break
            # 若输入为q，则将堆栈内还未输出的运算符输出
        elif infix_q[flag] == 'q':
            while stack_t[top] != 'q':
                print('%c' %stack_t[top], end='')
                top -= 1
            ...
            # 则将TOP的运算符输出，若大于等于TOP在堆栈
            # 中所指向的运算符，则将输入的运算符压入堆栈
